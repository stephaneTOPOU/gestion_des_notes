<?php
	...
	if(empty($_GET['evaluation']) || empty($_GET['lib']) ||  empty($_GET['typev']) ||  empty($_GET['date'])){
	...
	$reponse = $bdd->query('SELECT idsem,eval_noter,codemat from evaluation WHERE ideval=(\''.$_GET['evaluation'].'\')');
	while ($donnee = $reponse->fetch()) {
		$codemat = $donnee['codemat'];
		$idsem = $donnee['idsem'];
		$deja_eval = $donnee['eval_noter'];
	}
			...
			<div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="tab-content-details mg-b-30">
                            <h2><?php echo($_GET['lib']) ?></h2>
							<p>DATE : <b><?php echo($_GET['date']) ?></b></br>
							TYPE EVALUATION : <b><?php echo($_GET['typev']) ?></b></br> 
							<?php
								$reponse = $bdd->query('SELECT S.*, A.libellean FROM evaluation E, semestre S, annee A WHERE E.idsem=S.idsem AND A.idan=S.idan AND E.ideval=(\''.$_GET['evaluation'].'\')');
								while ($donnee = $reponse->fetch()) { 
									$sem=$donnee['idsem'];
								?>
							SEMESTRE : <b><?php echo($donnee['libellesem']) ?></b></br> ANNEE : <b><?php echo($donnee['libellean']) ?></b><p>
								<?php } ?>
                        </div>
                    </div>
					<?php
						$rep = $bdd->query('SELECT SUM(nbr_credi) AS nbr FROM matiere M,programmer P WHERE M.codemat=P.codemat AND P.idsem=(\''.$sem.'\') GROUP BY P.idsem');
						while ($donnee = $rep->fetch()) {
							$nbr_credi = $donnee['nbr'];
						}
						if($deja_eval!=null){
						?>
							<div class="col-lg-12 col-md-6 col-sm-6 col-xs-12">
								<div class="alert-icon shadow-inner wrap-alert-b">
									<div class="alert alert-warning alert-st-three" role="alert">
										<p class="message-mg-rt"><h4><strong>Alert !!</strong></h4><b>L'évaluation a déja été noté, Pour une modification veillez contacter la DAAS<b> <a href="indexEnseignant.php">Cliquez ici</a></p>
									</div>
								</div>
							</div>
						<?php
						}else if($nbr_credi != 30){
					?>
							<div class="col-lg-12 col-md-6 col-sm-6 col-xs-12">
								<div class="alert-icon shadow-inner wrap-alert-b">
									<div class="alert alert-danger alert-st-three" role="alert">
										<p class="message-mg-rt"><h4><strong>Urgent !!</strong></h4><b>Vous n'êtes pas autorisé a noter cette évaluation Contacter la DAAS<b> <a href="indexEnseignant.php">Cliquez ici</a></p>
									</div>
								</div>
							</div>
					<?php
					...
					?>
                <div class="row">
                    <div class="col-lg-12 col-md-16 col-sm-6 col-xs-12">
                        <div class="sparkline13-list">
                            <div class="sparkline13-graph">
                                <div class="datatable-dashv1-list custom-datatable-overright">
									<h4>Liste Des Participants</h4>
									<form action="Traitement_notation.php?evaluation=<?php echo($_GET['evaluation']);?>" method="POST">
                                    <table id="table" data-toggle="table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th data-field="email">Prénom</th>
                                                <th data-field="phone">Note</th>
                                            </tr>
                                        </thead>
                                        <tbody>
										<?php
											$id=0;
											$reponse = $bdd->query('SELECT E.idetu,E.nometu,E.prenometu,P.ideval FROM participer P, etudiant E,evaluation EV  WHERE EV.ideval=P.ideval AND  P.idetu=E.idetu AND P.ideval=(\''.$_GET['evaluation'].'\') AND EV.eval_noter IS NULL ORDER BY E.nometu ASC, E.prenometu ASC');
											while ($donnee = $reponse->fetch()) { $id=$id+1;?>
                                            <tr>
                                                <td></td>
                                                <td><?php echo($id); ?></td>
                                                <td><?php echo($donnee['nometu']); ?></td>
                                                <td><?php echo($donnee['prenometu']); ?></td>
                                                <td align="center"><input required maxlength="2" name="<?php echo($donnee['idetu']); ?>" type="text" onkeyup="verif_nombre(this);" class="form-control col-lg-6 col-md-9 col-sm-9 col-xs-12"/>
												</td>
                                            </tr>
											<?php } ?>
                                        </tbody>
										...
										<div class="button-style-four btn-mg-b-10">
											<input type="submit"  value="Valider les notes" onclick="return(confirm('Volez vous valider la notation ?'))" class="btn btn-custon-four btn-success"/>
											<input type="reset" value="Annuler la saisie" class="btn btn-custon-four btn-danger"/>
										</div>
									</form>
                                </div>
                            </div>
