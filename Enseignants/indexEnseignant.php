                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="tab-content-details mg-b-30">
                            <h2>VOTRE ENVIRONNEMENT DE TRAVAIL</h2>
                            <p>Interagissez librement avec l’application pour améliorer votre enseignement, consulter les statistiques et jugez par vous-même le résultat de vos durs labeurs<p>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-12 col-md-6 col-sm-6 col-xs-12">
                        <div class="admin-pro-accordion-wrap shadow-inner responsive-mg-b-30">
                            <div class="alert-title">
                                <h2>Liste Des UE Enseigné</h2>
                                <p></p>
                            </div>
                            <div class="panel-group edu-custon-design" id="accordion">
								<?php
								$reponse = $bdd->query('SELECT codemat, libellemat, nbr_credi FROM enseignant E, Matiere M WHERE E.idens=M.idens AND E.idens=(\''.$_SESSION['pseudo'].'\')');
								while ($donnee = $reponse->fetch()) {?>
                                <div class="panel panel-default">
                                    <div class="panel-heading accordion-head">
                                        <h4 class="panel-title">
                                            <a data-toggle="collapse" data-parent="#accordion" href="#collapse<?php echo($donnee['codemat']); ?>"><?php echo($donnee['libellemat']); ?> (<?php echo($donnee['nbr_credi']); ?> crédits)</a>
                                        </h4>
                                    </div>
                                    <div id="collapse<?php echo($donnee['codemat']); ?>" class="panel-collapse panel-ic collapse">
                                        <div class="panel-body admin-panel-content animated bounce">
											<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
												<div class="button-drop-style-one btn-default-bg">
													<button type="button" class="btn btn-custon-four btn-primary  dropdown-toggle1" data-toggle="dropdown"><b>Toutes les Evalutions</b> <i class="fa fa-angle-down"></i>
														</button>
													<ul class="dropdown-menu btn-dropdown-menu another-drop-pro-two dropdown-toggle1 sp-btn-dp-1" role="menu">
														<?php
														$repan = $bdd->query('SELECT * FROM annee');
														while ($d = $repan->fetch()) {?>
															<li><a href="toutes_eval.php?matiere=<?php echo($donnee['codemat']); ?>&annee=<?php echo($d['idan']); ?>">Année <?php echo($d['libellean']); ?></a><li>
														<?php } ?>
													</ul>
												</div>
											</div>
											<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
												<div class="button-drop-style-one btn-default-bg">
													<button type="button" class="btn btn-custon-four btn-primary  dropdown-toggle1" data-toggle="dropdown"><b>Voir Les notes</b> <i class="fa fa-angle-down"></i>
														</button>
													<ul class="dropdown-menu btn-dropdown-menu another-drop-pro-two dropdown-toggle1 sp-btn-dp-1" role="menu">
														<?php
														$repan = $bdd->query('SELECT * FROM annee');
														while ($d = $repan->fetch()) {?>
															<li><a href="statistiques.php?matiere=<?php echo($donnee['codemat']); ?>&annee=<?php echo($d['idan']); ?>">Année <?php echo($d['libellean']); ?></a><li>
														<?php } ?>
													</ul>
												</div>
											</div>
											...
											<p></p>
                                            <p><b></br>
											Evalution(s) Enrégistrée(s)</b></b></p>
											</div>
											<?php
											$rep = $bdd->query('SELECT ideval,date_eval,S.idsess,libelle,E.codemat FROM evaluation E, SESSION S WHERE E.idsess=S.idsess AND  E.codemat=(\''.$donnee['codemat'].'\') ORDER BY date_eval DESC LIMIT 4');
											while ($don = $rep->fetch()) {?>
											<a href="noter_evaluation.php?evaluation=<?php echo($don['ideval']); ?>&lib=<?php echo($donnee['libellemat']); ?>&typev=<?php echo($don['libelle']); ?>&date=<?php echo($don['date_eval']); ?>" title="noter ?">
												...
												<div class="hpanel shadow-inner hbgyellow bg-3 responsive-mg-b-30 res-tablet-mg-t-30 dk-res-t-pro-30">
													<div class="panel-body">
														<div class="text-center content-bg-pro">
															<h3><?php echo($don['libelle']); ?></h3>
															<p class="text-big font-light"><?php echo($don['date_eval']); ?></p>
														</div>
													</div>
												</div>
											</div>
